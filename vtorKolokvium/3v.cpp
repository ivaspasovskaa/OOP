...
public:
    Demonstrator(int indeks,int *ocenki, int brojOcenki,const char *imeIPrezime,Kurs *kursevi,int brojKursevi,int brojCasovi) : Student(indeks,ocenki,brojOcenki),Predavach(imeIPrezime,kursevi,brojKursevi){
...
Student& vratiNajdobroRangiran(Student ** studenti, int n ){
    Student *maxStudent=studenti[0];
    for(int i=0;i<n;i++){
        try{
            if(maxStudent->getBodovi()<studenti[i]->getBodovi()){
                maxStudent=studenti[i];
            }
        }
